def hex_to_bin(hex_string, bits):
    return bin(int(hex_string, 16))[2:].zfill(bits)
...
key = input("Enter 64-bit Key (HEX): ")
...
text = permute(plaintext_bin, IP)
print("\nAfter Initial Permutation:", text)
...
for i in range(16):
    ...
    for j in range(8):
        ...
        print(f"S{j+1} input: {block} -> output: {bin_val}")

        sbox_output += bin_val

    print("After S-Boxes:", sbox_output)

    p_output = permute(sbox_output, P)
    print("After P Permutation:", p_output)

    new_R = xor(L, p_output)
    L = R
    R = new_R
...
combined = R + L
cipher_bin = permute(combined, IP_INV)

print("\nAfter Final Permutation:", cipher_bin)

cipher_hex = bin_to_hex(cipher_bin)
print("\nFinal Ciphertext:", cipher_hex)
print("\n========= DECRYPTION =========")

rev_keys = subkeys[::-1]

text = permute(cipher_bin, IP)
L = text[:32]
R = text[32:]

for i in range(16):
    expanded = permute(R, E)
    xored = xor(expanded, rev_keys[i])
    ...
    for j in range(8):
        ...
        row = int(block[0] + block[5], 2)
        col = int(block[1:5], 2)
        sbox_output += bin(SBOX[j][row][col])[2:].zfill(4)
...
combined = R + L
plain_recovered = permute(combined, IP_INV)

print("Recovered Plaintext:", bin_to_hex(plain_recovered))
